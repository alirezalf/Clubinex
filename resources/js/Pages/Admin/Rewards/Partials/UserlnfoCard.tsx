            ...
            <div className="flex flex-col md:flex-row gap-8 relative z-10">
                <div className="flex-1 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm mt-4">
                        ...
                        {user.username && (
                            <div className="flex items-center gap-2 text-gray-600 bg-gray-50 p-2 rounded-lg">
                                <span className="text-gray-400 shrink-0 font-medium">@</span> <span className="font-mono text-gray-800">{user.username}</span>
                            </div>
                        )}
                        {user.email && (
                            <div className="flex items-center gap-2 text-gray-600 bg-gray-50 p-2 rounded-lg">
                                <span className="font-mono text-gray-800 truncate">{user.email}</span>
                            </div>
                        )}
                ...
                <div className="flex-1 flex flex-col justify-center gap-4">
                    <div className="flex items-center justify-between bg-primary-50 p-3 rounded-xl border border-primary-100">
                        ...
                        <span className="text-2xl font-black text-primary-600 font-mono tracking-tight">{(user.current_points || 0).toLocaleString()}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                        <div className="bg-green-50 p-2.5 rounded-xl text-green-800 text-center border border-green-100">
                            <div className="font-black text-base mb-1 font-mono">+{(stats?.total_earned || 0).toLocaleString()}</div>
                        ...
                        <div className="bg-red-50 p-2.5 rounded-xl text-red-800 text-center border border-red-100">
                            <div className="font-black text-base mb-1 font-mono">-{(stats?.total_spent || 0).toLocaleString()}</div>
