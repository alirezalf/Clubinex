...
export default function SlideItem({ slide, isActive }: Props) {
    const titleColor = slide.text_color && (slide.text_color.startsWith('#') || slide.text_color.startsWith('rgb'))
        ? slide.text_color
        : undefined;

    const titleClass = !titleColor ? (slide.text_color || 'text-white') : '';

    // Description Color Logic
    const descColor = slide.desc_color && (slide.desc_color.startsWith('#') || slide.desc_color.startsWith('rgb'))
        ? slide.desc_color
        : titleColor; // Fallback to title color if custom desc color not set

    // If no custom desc color AND title has no custom color (uses class), use class inheritance or default
    const descClass = !descColor && !titleColor ? (slide.text_color || 'text-white') : '';
            ...
            <div className={`absolute inset-0 flex p-6 md:p-12 z-10 h-full w-full ${wrapperClass(slide.content_position)}`}>
                <div className={`max-w-3xl flex flex-col ${slide.content_position.includes('center') ? 'items-center' : (slide.content_position.includes('right') ? 'items-start' : 'items-end')}`}>
                    ...
                    <div className={`${textAnimClass}`}>
                        {slide.title && (
                            <h2
                                className={`font-black drop-shadow-lg leading-tight ${slide.text_size || 'text-3xl'} ${titleClass}`}
                                style={{
                                    color: titleColor,
                                    marginBottom: slide.gap_y ? `${slide.gap_y}px` : '1rem' // Applied user defined gap
                                }}
                            >
                                {slide.title}
                            </h2>
                        ...
                        {slide.description && (
                            <p
                                className={`opacity-95 drop-shadow-md leading-relaxed font-medium mb-6 ${slide.desc_size || 'text-base'} ${descClass}`}
